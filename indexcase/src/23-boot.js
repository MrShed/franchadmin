...
var UIBoot = {
  title: function () {
    ...
    root.innerHTML = '<div id="t-cv" style="position:absolute;inset:0"></div><div class="t-shade"></div><div class="t-wrap">' +
      ...
      '<p class="t-dek">A disease nobody has seen before appears in a British city. You lead the public health response. Work out what it is while it spreads — and decide what to do before you are sure.</p></div>' +
      '<div class="t-panel">' +
  ...
  backdrop: function () {
    var city = UIMapR.fakeCity(7), host = UI$('#t-cv'), t0 = performance.now();
    ...
    var cv = UIcanvas(host, function (ctx, w, h) {
      ...
      var el = ((performance.now() - t0) / 1000) % 26, pts = [];
    ...
    function loop() { if (UI$('#title').hidden || !host.isConnected) { if (cv.ro) cv.ro.disconnect(); return; } cv.frame(); UIBoot._raf = setTimeout(function () { requestAnimationFrame(loop); }, 33); }
