...
function fit() {
  ...
  const kbUp = touch && document.activeElement && document.activeElement.id === 'kb';
  // orientation from the device, not the window: the phone keyboard shrinks the window and must not flip the layout
  const land = touch && isLand(); document.body.classList.toggle('land', land); document.body.classList.toggle('kbup', kbUp);
  const kbBtn = document.getElementById('k-kb'); if (kbBtn) kbBtn.textContent = kbUp ? 'HIDE' : 'KEYS';
  // landscape: controls sit either side of the screen; portrait: under it (only ESC/TAB/HIDE while the keyboard is up)
  const vv = window.visualViewport, H0 = kbUp && vv ? vv.height : window.innerHeight;
  const vw = window.innerWidth - (land ? 2 * Math.min(190, Math.max(150, window.innerWidth * 0.2)) : 0), vh = H0 - (touch && !land ? (kbUp ? 60 : 190) : 0);
...
function isLand() { const o = screen.orientation && screen.orientation.type; if (o) return o.startsWith('landscape'); if (typeof window.orientation === 'number') return Math.abs(window.orientation) === 90; return window.innerWidth > window.innerHeight; }
...
let kbHidden = false; // the player tapped HIDE: stay down until they ask again or a new typing screen opens
function kbOpen(force) { if (force) kbHidden = false; if (!typing || !document.body.classList.contains('touch') || ((isLand() || kbHidden) && !force)) return; if (document.activeElement !== kb) { kbReset(); kb.focus({ preventScroll: true }); } }
...
// KEYS raises the keyboard (the only way in landscape, where it would cover the screen); HIDE drops it
document.getElementById('k-kb').addEventListener('pointerdown', e => { e.preventDefault(); sfx.unlock(); if (document.activeElement === kb) { kbHidden = true; kb.blur(); } else kbOpen(true); });
const refit = () => { fit(); setTimeout(fit, 120); setTimeout(fit, 400); };
kb.addEventListener('focus', refit); kb.addEventListener('blur', refit);
...
function kbSync() {
  ...
  if (want && !kbWanted) { kbHidden = false; kbOpen(); }
